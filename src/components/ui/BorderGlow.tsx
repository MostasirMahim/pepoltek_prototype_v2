...
function parseHSL(hslStr: string): { h: number; s: number; l: number } {
  if (!hslStr) return { h: 212, s: 100, l: 52 };

  // Hex support for maximum flexibility
  if (hslStr.startsWith("#")) {
    const hex = hslStr.replace("#", "");
  ...
  // Matches "40 80 80", "212, 100%, 52%", "hsl(212 100 52)"
  const match = hslStr.match(/([\d.]+)\s*,?\s*([\d.]+)%?\s*,?\s*([\d.]+)%?/);
  if (!match) return { h: 212, s: 100, l: 52 };
...
function animateValue({
  ...
  const timeoutId = setTimeout(() => {
    if (!isCancelled) animId = requestAnimationFrame(tick);
  }, delay);
...
export const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className={contentClassName || "relative z-[1] flex flex-col overflow-hidden"}>
        {children}
      </div>
    </div>
  );
...
/**
 * GlowButton: High-performance interactive button wrapper built with BorderGlow.
 * Supports link routing via href or standard button clicks, optimized for button proportions.
 */
...
export const GlowButton: React.FC<GlowButtonProps> = ({
  ...
  // Preset themes calibrated for Pepoltek enterprise design system
  ...
  const defaultGlow = glowColor || "212 100 52";
  const defaultColors = colors || ["#0a84ff", "#38bdf8", "#60a5fa"];

  const innerContent = (
    <div
      className={`inline-flex items-center justify-center gap-1.5 px-5 py-2.5 font-display text-xs sm:text-sm font-semibold transition-colors duration-200 select-none ${
        isPrimary
          ? "text-white group-hover:text-white"
          : "text-[#0a1428] group-hover:text-electric"
      } ${contentClassName}`}
    >
      {children}
    </div>
  ...
  const glowWrapper = (
    <BorderGlow
      ...
      animated={animated}
      className={`group inline-block transition-transform duration-200 hover:-translate-y-0.5 cursor-pointer shadow-sm ${
        isOutline ? "border-[#bcd6fa]" : ""
      } ${containerClassName}`}
      contentClassName="relative z-[1] w-full h-full"
    >
      {innerContent}
    </BorderGlow>
  );
  ...
  if (href) {
    if (href.startsWith("/")) {
      return (
        <Link href={href} className={`inline-block no-underline ${className}`}>
          {glowWrapper}
        </Link>
      );
    }
    return (
      <a href={href} className={`inline-block no-underline ${className}`}>
        {glowWrapper}
      </a>
    );
  }
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`inline-block border-none bg-transparent p-0 cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-electric rounded-[${borderRadius}px] ${className}`}
      {...buttonProps}
    >
      {glowWrapper}
    </button>
